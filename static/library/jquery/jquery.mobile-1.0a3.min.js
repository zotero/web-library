...
G()}function B(H){a.mobile.pageContainer.addClass(H);E.push(H)}a.mobile.silentScroll();var N=f.scrollTop(),K=["flip"],E=[];if(x.indexOf("&"+a.mobile.subPageUrlKey)>-1)l=a("[data-url='"+x+"']");if(y){y.data("lastScroll",N);y.data("page")._trigger("beforehide",{nextPage:l})}l.data("page")._trigger("beforeshow",{prevPage:y||a("")});if(s&&s!=="none"){a.mobile.pageLoading(true);a.inArray(s,K)>=0&&B("ui-mobile-viewport-perspective");B("ui-mobile-viewport-transitioning");if(y)y.addClass(s+" out "+(q?"reverse":
""));l.addClass(a.mobile.activePageClass+" "+s+" in "+(q?"reverse":""));l.animationComplete(function(){l.add(y).removeClass("out in reverse "+s);y&&y.removeClass(a.mobile.activePageClass);A();a.mobile.pageContainer.removeClass(E.join(" "));E=[]})}else{a.mobile.pageLoading(true);y&&y.removeClass(a.mobile.activePageClass);l.addClass(a.mobile.activePageClass);A()}}function S(){if(j||l.data("role")==="dialog"){x=h.getActive().url+t;if(j){l.attr("data-role",j);j=null}}l.page()}var F=a.type(l)==="array",
I=a.type(l)==="object",y=F?l[0]:a.mobile.activePage;l=F?l[1]:l;var x=a.type(l)==="string"?e.stripHash(l):"",C=x,D,O="get",T=false,M=null,P=h.getActive(),J=false,L=false;if(!(P&&h.stack.length>1&&P.url===x&&!F&&!I))if(p)o.unshift(arguments);else{p=true;if(z){a.each(h.stack,function(A){if(this.url===x){urlIndex=A;J=A<h.activeIndex;L=!J;h.activeIndex=A}});if(J){q=true;s=s||P.transition}else if(L)s=s||h.getActive().transition}if(I&&l.url){x=l.url;D=l.data;O=l.type;T=true;if(D&&O==="get"){if(a.type(D)===
"object")D=a.param(D);x+="?"+D;D=d}}base&&base.reset();a(window.document.activeElement).add("input:focus, textarea:focus, select:focus").blur();if(x){l=a("[data-url='"+x+"']");C=e.getFilePath(x)}else{F=l.attr("data-url");I=e.getFilePath(F);if(F!==I)C=I}if(s===d)s=a.mobile.defaultTransition;if(l.length&&!T){C&&base&&base.set(C);S();Q()}else{if(l.length)M=l;a.mobile.pageLoading();a.ajax({url:C,type:O,data:D,success:function(A){var B=/ data-url="(.*)"/.test(A)&&RegExp.$1;if(B){base&&base.set(B);x=C=
e.getFilePath(B)}else base&&base.set(C);B=a("<div></div>");B.get(0).innerHTML=A;l=B.find('[data-role="page"], [data-role="dialog"]').first();if(!a.support.dynamicBaseTag){var N=e.get(C);l.find("[src],link[href]").each(function(){var K=a(this).is("[href]")?"href":"src",E=a(this).attr(K);E.replace(location.protocol+"//"+location.host+location.pathname,"");/^(\w+:|#|\/)/.test(E)||a(this).attr(K,N+E)})}l.attr("data-url",C).appendTo(a.mobile.pageContainer);S();setTimeout(function(){Q()},0)},error:function(){a.mobile.pageLoading(true);
c(true);base&&base.set(e.get());a("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>Error Loading Page</h1></div>").css({display:"block",opacity:0.96,top:a(window).scrollTop()+100}).appendTo(a.mobile.pageContainer).delay(800).fadeOut(400,function(){a(this).remove()});G()}})}}};a("form[data-ajax!='false']").live("submit",function(l){if(a.mobile.ajaxEnabled&&a.mobile.ajaxFormsEnabled){var s=a(this).attr("method"),q=e.clean(a(this).attr("action"));if(!e.isExternal(q)){if(e.isRelative(q))q=
