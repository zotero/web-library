...
const iconsPath = 'chrome/skin/default/zotero/item-type/';
...
(async () => {
    ...
    await exec(
        ...
        + ` && cd zotero`
        + ` && git sparse-checkout add ${iconsPath}`
        + ` && git checkout`
    );
    for(let resolution of resolutions) {
        const srcIconDir = join(tmpDir, 'zotero', iconsPath, resolution);
        const targetIconDir = join(ROOT, 'src', 'static', 'icons', resolution, 'item-type');
        let map = new Map();
        ...
        for(let color of colors) {
            ...
            for(let file of iconFiles) {
                ...
                try {
                    const pixelRatio = file.includes('@2x') ? '2x' : '1x';
